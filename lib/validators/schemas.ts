 ...
 * `refine`) is used so the specific reason — unparseable, wrong scheme, or
 * wrong host — reaches the caller instead of one catch-all message, and
 * `path: ['url']` keeps the issue attached to the field the admin has to fix.
 */
...
// -----------------------------------------------------------------------
// SubmissionSchema
// Validates a project submission URL and participant details. The URL must
// be on the partyrock.aws domain (or a subdomain of it).
// -----------------------------------------------------------------------
...
// pastes or corrects a project's widget configuration/prompts after
// submission (the primary evidence the AI scorer reads).
// -----------------------------------------------------------------------
