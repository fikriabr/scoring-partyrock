...
import type { PartyRockMetadata, ScoringParameter, ScoringResult } from '@/types'
...
export class ScorerService {
  ...
  static async triggerScoring(projectId: string): Promise<void> {
    ...
    const metadata: PartyRockMetadata = project.metadata
      ? {
        title: project.metadata.title,
        description: project.metadata.description,
        widgets: project.metadata.widgets as unknown as PartyRockMetadata['widgets'],
    ...
    const contextProjects: PartyRockMetadata[] = siblingMetadataRows.map((m) => ({
      title: m.title,
      description: m.description,
      widgets: m.widgets as unknown as PartyRockMetadata['widgets'],
